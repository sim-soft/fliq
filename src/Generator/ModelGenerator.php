 ...
 * properties, casts, fillable fields, and trait usage inferred from schema.
 */
class ModelGenerator
{
     ...
     *
     * @return string|false The file path written, or false if skipped (file exists and force=false).
     */
    ...
    /**
     * Generate the model class code as a string (without writing to file).
     *
    ...
    private function introspectSQLite(): array
    {
        ...
        foreach ($rows as $row) {
            $columns[] = [
                'name' => $row['name'],
                'type' => $this->normalizeType($row['type'] ?? 'text'),
    ...
    private function buildClassCode(string $className, array $columns): string
    {
        ...
        $fillable = $this->extractFillable($columns, $primaryKey);
        $casts = $this->extractCasts($columns, $primaryKey);
        ...
        if ($primaryKey !== 'id') {
            ...
            $lines[] = "    protected string|array \$primaryKey = '$primaryKey';";
        }
     ...
     * @return string
     */
    private function extractPrimaryKey(array $columns): string
    {
    ...
    /**
     * Extract fillable columns (excludes PK, timestamps, soft delete).
     *
     * @param array<int, array{name: string, type: string, nullable: bool, default: mixed, primary: bool}> $columns
     * @param string $primaryKey The primary key name.
     * @return array<int, string>
     */
    private function extractFillable(array $columns, string $primaryKey): array
    {
        $excluded = [$primaryKey, 'created_at', 'updated_at', 'deleted_at', 'created', 'updated'];
        $fillable = [];
     ...
     */
    private function extractCasts(array $columns, string $primaryKey): array
    {
        $excluded = [$primaryKey, 'created_at', 'updated_at', 'deleted_at', 'created', 'updated'];
        $casts = [];
    ...
    /**
     * Build PHPDoc @param array<int, array{name: string, type: string, nullable: bool, default: mixed, primary: bool}> $columns
     * @return array<int, string>
