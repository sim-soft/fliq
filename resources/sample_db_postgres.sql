-- ============================================================
-- sample_db — PostgreSQL test schema for simsoft/fliq ORM
...
CREATE TABLE "user" (
    ...
    role        VARCHAR(10) NOT NULL DEFAULT 'member' CHECK (role IN ('admin','editor','member')),
...
-- Full-text search index (PostgreSQL uses GIN for tsvector)
CREATE INDEX idx_post_fts ON post USING GIN (to_tsvector('english', title || ' ' || body));
...
SELECT setval('order_item_id_seq', 38);
...
SELECT setval('setting_id_seq', 10);
