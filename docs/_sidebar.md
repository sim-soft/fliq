- **Introduction**
    - [About](/)

- **Getting Started**
    - [Connections & Drivers](01-GETTING-STARTED.md)

- **Query Builder**
    - [Fluent API](02-QUERY-BUILDER.md)

- **Active Record**
    - [Models & CRUD](03-ACTIVE-RECORD.md)

- **Relations**
    - [hasOne / hasMany / M:N](04-RELATION.md)

- **Advanced Features**
    - [Caching, Pagination, Batch](05-ADVANCED-FEATURES.md)

- **Collections**
    - [filter, map, reduce, groupBy](06-COLLECTIONS.md)

- **Comparison**
    - [FLIQ vs Other ORMs](07-COMPARISON.md)

- **Quick Reference**
    - [Cheatsheet](08-CHEATSHEET.md)

<div style="text-align:center;margin:20px 0">
    <a href="https://github.com/sim-soft" target="_blank" style="text-decoration:none">
        <img src="img/logo/simsoft.webp" alt="Simsoft" width="100">
        <p class="logo-text" style="margin-top:8px;font-size:0.85em;text-decoration:none">by Simsoft</p>
    </a>
</div>
